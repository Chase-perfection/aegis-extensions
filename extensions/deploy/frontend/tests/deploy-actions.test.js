...
const DEPLOY_ERROR_TEXT = {
    ...
    no_index: 'No index.html at the root of that branch, but a subfolder has one. The build log above names it. Put that folder in the subfolder field and deploy again.',
    not_a_site: 'This branch holds no web page anywhere, so there is nothing to serve as files. If it is an application, give it a start command and Aegis runs it instead. If its pages are built from source, fill in the build command and the output directory.',
    ...
    bad_deploy_manifest: 'The aegis.deploy.json in that branch could not be read. Aegis refuses rather than create a project whose declared settings it ignored.',
...
const REFUSAL_REFERENCE = [
    ...
    { code: 'no_index', name: 'The site is in a subfolder, not at the root' },
    { code: 'not_a_site', name: 'The branch holds no site and no source to build' },
    ...
    { code: 'bad_deploy_manifest', name: 'The aegis.deploy.json in that branch will not parse' },
